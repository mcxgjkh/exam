...
(function() {
    ...
    function shouldShowModal() {
        ...
        const defaultVersion = '3.10.4.20260504_beta.2';
    ...
    let currentPage = 1;
    const pageSize = 10;
    let historyList = [];          // 存储所有历史记录
    let historyCollapsed = false;  // 折叠状态
    ...
    function renderHistoryList() {
        ...
        // 绑定“查看错题”按钮事件（使用委托或直接绑定）
        container.querySelectorAll('.history-view-wrong').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const idx = parseInt(btn.dataset.idx);
                if (!isNaN(idx)) window.showWrongDetailModal(idx);
            });
        });
    }
    ...
    // 错题详情模态框
    function showWrongDetailModal(recordIndex) {
        const record = historyList[recordIndex];
        if (!record) return;
        const bank = questionBanks[record.type];
        if (!bank) {
            alert('无法加载题库，请刷新后重试');
            return;
        }

        // 根据 wrongIds 获取题目完整对象，同时获取对应的用户答案
        ...
        record.wrongIds.forEach((id, idx) => {
            ...
            if (q) {
                ...
                wrongAnswers.push(record.wrongUserAnswers ? record.wrongUserAnswers[idx] : null);
            }
        });
        ...
        contentHtml += `<h3 style="margin-top:0;">${record.type}类考试 错题详情 (${wrongQuestions.length}题)</h3>`;
        wrongQuestions.forEach((q, idx) => {
            let questionText = q.question.replace(/\[image:\s*([^\]]+)\]/g, (match, filename) => {
                return `<span class="modal-image-placeholder" data-img-src="imageswebp/${filename}"></span>`;
            });
            // 选项（按原始顺序）
            let optionsHtml = '';
            q.options.forEach((opt, optIdx) => {
                const letter = String.fromCharCode(65 + optIdx);
                optionsHtml += `<div style="margin: 4px 0;">${letter}. ${opt.text}</div>`;
            });
            // 正确答案（转换成字母）
            let correctDisplay = '';
            ...
            const userAns = wrongAnswers[idx];
            if (userAns !== undefined && userAns !== null) {
                if (Array.isArray(userAns)) {
                    // 多选题：数组中的每个 value 本身就是字母（如 'A','B'）
                    userAnswerDisplay = userAns.join('、');
                } else {
                    userAnswerDisplay = userAns;
                }
            }
            ...
            contentHtml += `
                <div style="border: 1px solid #ddd; border-radius: 12px; padding: 12px; margin-bottom: 16px; background: #fafdff;">
                    ...
                    <div class="modal-options" style="margin-left: 16px;">${optionsHtml}</div>
                    <div style="margin-top: 8px; color: #c44536;"><strong>您的答案：</strong> ${userAnswerDisplay}</div>
                    <div style="margin-top: 4px; color: #2e7d32;"><strong>正确答案：</strong> ${correctDisplay}</div>
                </div>
            `;
        });
        contentHtml += `<div style="text-align:center; margin-top:20px;"><button id="close-wrong-modal" class="small-btn">关闭</button></div></div>`;

        // 移除已存在的模态框（避免重复）
        let existingModal = document.getElementById('wrong-detail-modal');
        ...
        fixSpecialCharacters(modal);

        // 处理模态框中的图片占位符（异步加载）
        const placeholders = modal.querySelectorAll('.modal-image-placeholder');
        ...
        // 绑定关闭事件
        const closeBtn = modal.querySelector('.wrong-detail-close');
        const closeInnerBtn = modal.querySelector('#close-wrong-modal');
    ...
    window.showWrongDetailModal = showWrongDetailModal;

    // 查看错题详情（弹出提示框显示错题ID）
    // window.viewWrongList = function(index) {
    //     const record = historyList[index];
    //     if (record && record.wrongIds && record.wrongIds.length) {
    //         alert(`错题ID列表：\n${record.wrongIds.join('\n')}`);
    //     } else {
    //         alert('本次考试没有错题');
    //     }
    // };
    ...
    function deleteSelectedHistory(selectedIndices) {
        // 从后往前删除避免索引错乱
        for (let i = selectedIndices.length - 1; i >= 0; i--) {
            historyList.splice(selectedIndices[i], 1);
        }
        saveHistory();
        renderHistoryList();
    }
    ...
    function showQuestion(index) {
        ...
        // 修复当前题目区域中的特殊字符字体
        fixSpecialCharacters(document.getElementById('question-container'));
    ...
    function submitExam() {
        ...
        const timeUsedStr = `${minutes.toString().padStart(2,'0')}:${seconds.toString().padStart(2,'0')}`;
        addHistoryRecord({
            type: currentExamType,
            score: correctCount,
            total: currentQuestions.length,
            passed: correctCount >= examConfig[currentExamType].pass,
            timeUsedSec: used,
            timeUsedStr: timeUsedStr,
            wrongIds: wrongIds,
    ...
    // 特殊字符列表
    const SPECIAL_CHARS = /[\p{Script=Greek}\p{So}\p{Sm}\u03B1-\u03C9\u03A9\u03A0\u03A3\u0394\u0398\u00B0\u2103\u2109\u2032\u2033\u00B2\u00B3\u00B9\u2070-\u2079\u2080-\u2089\u00D7\u00F7\u2260\u2264\u2265\u221E\u221A\u2211\u220F\u2202\u2207\u2220\u221D\u2190\u2192\u2191\u2193\u2194\u21D0\u21D2\u21D4\u00B1\u220F\u2211\u222B\u222C\u222E\u220F\u221E\u00BD\u00BC\u00BE\u00A9\u00AE\u2122\u20AC\u00B5\u03BC]/gu;

    function fixSpecialCharacters(container) {
        if (!container) return;
        // 用于存储需要处理的文本节点及其上下文信息
        const nodesToProcess = [];
        const walker = document.createTreeWalker(container, NodeFilter.SHOW_TEXT, {
            acceptNode: function(node) {
                // 跳过已经被 fallback 包裹的节点（防止重复处理）
                if (node.parentElement && node.parentElement.classList.contains('fallback-char')) {
                    return NodeFilter.FILTER_REJECT;
                }
                // 每次 test 前重置正则的 lastIndex（避免全局标志干扰）
                SPECIAL_CHARS.lastIndex = 0;
                if (SPECIAL_CHARS.test(node.textContent)) {
        ...
        nodesToProcess.forEach(textNode => {
            ...
            if (!parent) return;
            // 检查父元素是否为 mark（高亮标签），如果是，则保留 mark 但内部包裹 fallback
            const isMark = parent.tagName === 'MARK';
            const fragment = document.createDocumentFragment();
            let lastIndex = 0;
            SPECIAL_CHARS.lastIndex = 0;
            let match;
            while ((match = SPECIAL_CHARS.exec(originalText)) !== null) {
                ...
                if (isMark) {
                    // 如果原父元素是 mark，则需要将 fallback 包裹的字符保留在 mark 内
                    // 此处先添加到 fragment，后面统一将 fragment 包裹进 mark
            ...
            // 如果父元素是 mark，则需要将 fragment 内的所有节点重新包裹到一个新的 mark 元素中
            if (isMark) {
                const newMark = document.createElement('mark');
                newMark.appendChild(fragment);
                parent.parentNode.replaceChild(newMark, parent);
            } else {
    ...
    function calculateScore(question, type, rawKeyword) {
        ...
        // 拼音首字母子串匹配（得分80，只要输入拼音是题干拼音的子串）
        const pinyinTarget = getPinyinInitials(altText);
        let pinyinScore = 0;
        if (cleanKeyword.length > 0 && pinyinTarget.includes(cleanKeyword)) {
            pinyinScore = 80;
        }
        const fuzzyScore = textSimilarity * 50 + pinyinScore;
    ...
    async function performQuery() {
        ...
        setTimeout(() => {
            ...
            const topResults = scoredResults.slice(0, 100);
            ...
            const showResults = () => {
                ...
                displayQueryResults(topResults, keyword);
                queryPending = false;
                queryBtn.disabled = false;
                queryBtn.textContent = '查询';
                queryTimer = null;
    ...
    function renderCurrentPage() {
        const paginationContainer = document.getElementById('query-pagination-top');
        const resultsContainer = document.getElementById('query-results');
        if (!paginationContainer || !resultsContainer) return;
        ...
        if (totalItems === 0) {
            resultsContainer.innerHTML = '<p style="text-align:center; color:#7f8c8d;">未找到匹配的题目</p>';
        ...
        pageItems.forEach(({ question: q, types }) => {
            ...
            // 处理题干：分离图片标记和纯文本，只对纯文本部分高亮
            let questionText = q.question;
            let parts = [];
            let imgRegex = /\[image:\s*([^\]]+)\]/g;
            let lastIdx = 0;
            let m;
            while ((m = imgRegex.exec(questionText)) !== null) {
                let before = questionText.slice(lastIdx, m.index);
                parts.push(before);
                parts.push(`<span class="image-placeholder-temp" data-img-src="imageswebp/${m[1]}"></span>`);
                lastIdx = m.index + m[0].length;
            }
            parts.push(questionText.slice(lastIdx));
            // 对每个纯文本部分进行高亮
            for (let i = 0; i < parts.length; i++) {
                if (typeof parts[i] === 'string' && !parts[i].includes('image-placeholder-temp')) {
                    parts[i] = highlightText(parts[i], keyword);
                }
            }
            let questionHtml = parts.join('');
            // 恢复图片占位符
            ...
            q.options.forEach((opt, i) => {
                ...
                optText = highlightText(optText, keyword);
        ...
        fixSpecialCharacters(resultsContainer);

        // 绑定分页事件
        bindPaginationEvents(totalPages);
        // 绑定图片占位符检测（针对新插入的结果）
        attachImagePlaceholders(resultsContainer);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        loadHistory();
        fixSpecialCharacters(document.body);
        ...
        if (modal) {
            if (!shouldShowModal()) {
                ...
                if (lastVersionSpan && currentVersionSpan) {
                    ...
                    const versionElem = document.querySelector('.version');
                    const rawText = versionElem ? versionElem.textContent : '版本号：3.10.4.20260504_beta.2';
                    const match = rawText.match(/[\d.]+[_\w.]*/);
                    const currentVer = match ? match[0] : '3.10.4.20260504_beta.2';
                ...
                const currentVersion = versionMatch ? versionMatch[0] : '3.10.4.20260504_beta.2';
                const agreed = document.cookie.split(';').some(c => c.trim().startsWith('agree_policy=true'));
        ...
        const closeBtn = previewModal.querySelector('.image-preview-close');
