...
(function() {
    ...
    async function performQuery() {
        ...
        const keyword = rawInput;  // 保留原始输入用于评分，不转义HTML
        ...
        // 按得分降序排序，得分相同时按 ID 排序
        scoredResults.sort((a, b) => {
            if (a.score !== b.score) return b.score - a.score;
            return a.question.id.localeCompare(b.question.id);
        });

        displayQueryResults(scoredResults);
    }

    function displayQueryResults(scoredResults) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (modal) {
            if (!shouldShowModal()) {
                ...
                const versionElem = document.querySelector('.version');
                const rawText = versionElem ? versionElem.textContent : '版本号：2.15.1';
                const versionMatch = rawText.match(/[\d.]+/);
                const currentVersion = versionMatch ? versionMatch[0] : '2.15.1';
                const agreed = document.cookie.split(';').some(c => c.trim().startsWith('agree_policy=true'));
